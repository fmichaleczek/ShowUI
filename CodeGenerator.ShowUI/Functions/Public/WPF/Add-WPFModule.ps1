function Add-WPFModule {
    ...
    #.Example
    #  Add-WPFModule -Path .\MahApps.Metro.dll -Name MahApps.Metro
    #
    #   The simplest way to use Add-WPFModule is to specify the path (or assembly name, if it's in the GAC) to a dll with WPF controls in it, and the name of the module that you want to create.
    #   
    #.Example
    #   Add-WPFModule -Path .\MahApps.Metro.dll -Name ~\Documents\WindowsPowerShell\MahApps\MahApps.psm1
    #
    ...
    process {
        if ($psCmdlet.ParameterSetName -eq 'Type') {
            ...
            [Type[]]$filteredTypes = Select-WPFType -Path @($Path) -AssemblyName @($AssemblyName) -TypeNameWhiteList @($TypeNameWhiteList) -TypeNameBlackList @($TypeNameBlackList)
        }
